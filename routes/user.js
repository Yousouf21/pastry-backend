...
// PUT - add order --------------------------------------
router.put('/addPastryOrder/', Utils.authenticateToken, (req, res) => {  
  // validate check
  if(!req.body.orderId){
  ...
  // add orderId to orders field (array - push)
  User.updateOne({
    _id: req.user._id
  }, {
    $push: {
      pasOrders: req.body.orderId
    }
  })
    .then((user) => {            
      res.json({
...
router.get('/:id', Utils.authenticateToken, (req, res) => {
  ...
  User.findById(req.params.id).populate('pasOrders')
    .then(user => {
      res.json(user)
    })
    .catch(err => {
